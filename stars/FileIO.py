...
class csvWriter:
    def __init__(self, fileName, info, delimiter=','):
        """Class that writes csv files.写入csv文件的类
        arguments:
            fileName = (str) = name of the output file 输出文件的文件
            info = (list of lists) each list will be treated as a row in the
            output file.每个列表将被视为一行输出文件。  
            Each item in each list will be seperateed by a comma.
            每个列表中的每项都将被逗号分隔。
        """
