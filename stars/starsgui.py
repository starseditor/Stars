...
class  SProject(Project):
   ...
   """
    def __init__(self, name, master, app):
        ...
        self.master = master
        self.app = app
        self.fnt=("Times",10) #代表字体
        self.screenHeight = self.app.winfo_screenheight()  #屏幕高度
        self.screenWidth = self.app.winfo_screenwidth()  #屏幕宽度
...
class App(GuiMixin, GuiMaker):   # or GuiMakerFrameMenu
    ...
    def start(self): #开始界面的对话框
        ...
        self.master.bind("<Control-q>", self.quit) #退出的绑定设置
        ...
        self.menuBar = [                               
          ('File', 0,                                  # (pull-down)
              [#('New Project ...',  0, self.newProject),           # [menu items list]
               ('Open Shapefile',0,self.createNewSTARSProject),
          ...
          (
              'Operate Track',0,
              [
                  ('Start Track',0,self.startTrack),
                  ('Finish Track',0,self.finishTrack),
                  'separator',
                  ('Load Model', 0, self.loadmodel)
              ]
          ),
          ('Help',0,
    ...
    def loadmodel(self):
        ...
        filename = askopenfilename(filetypes=[("STARS Model", "*.opr")])
        ...
        #         line=file.readline()
        #         funname.append(line)
        #     self.trace={name:funname}
        #     line.readline()
        # for dex in len(self.names):
        #     txt=self.names[dex]
        #     button = Button(root, bg="White", text=txt, width=5, height=1, relief=GROOVE,
        #                     command=lambda index=dex, n=txt: self.tracebutton(index, n))
        ...
        if line:
            ...
            for index in range(num):
                ...
                line = line.rstrip('\n')
                if line:
                    self.buttonname.append(line.split(',')[0])
                    label = Label(self.root, text=line.split(',')[0])
                    label.pack()
                    self.funname.append(line.split(',')[1])
        OKbtn = Button(self.root, text='OK', command=self.masterquits)
        OKbtn.pack()
        ...
        if len(self.funname):
            for index in range(num):
                ...
                label = Label(self.root, text=self.buttonname[index])
                label.pack()
                OKbtn = Button(self.root, text='OK', command=self.masterquits)
                OKbtn.pack()
                self.root.wait_window()
                name = self.funname[index]
                getattr(self, name)()

    def tracebutton(self, index, name):
        for buttonname in self.trace[name]:
            getattr(buttonname)()
    def error(self,string):
        so = "STARS Error: %s"%string
        self.report(so)

    def masterquits(self):
        self.root.destroy()
    ...
    def disableMenus(self):
        try:
            ...
            fileMenu.entryconfigure(6,state=DISABLED)
            #fileMenu.entryconfigure(5,state=DISABLED)
        except:
            pass
    ...
    # File callbacks
    ######################################## 
    def Quit(self,trace=1):
        # added to deal with shell+gui options
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate,"Quit","Quit")
        self.quit
        self.master.destroy()
        sys.exit(0)

    def example(self,trace=1):
        ...
        self.enableMenus()
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Example Project", "example")
    ...
    def createNewSTARSProject(self,trace=1):
        """
        ...
        if baseFileName:
            ...
            if d.status:
                ...
                newFileName=d.results[0]
            self.baseVariableTable()
            newfile = os.path.dirname(baseFileName) + '/'+newFileName+'.prj'
            d = sd.SDialogue('Choose Time Series Type')
            values = 'Decadal', 'Annual', 'Quarterly', 'Monthly', 'Irregular'
            txt = "Choose the type of file you want to use as yourc base data.\n"
            ...
            self.enableMenus()
            self.d = Tkinter.Toplevel()
            lableframe1 = LabelFrame(self.d, text="Convert")
            lableframe1.pack()
            btcs = Button(lableframe1, text='BaseTOCS', command=self.convertCSVariables)
            btcs.grid();
            btcsts = Button(lableframe1, text='BaseTOCSTS', command=self.convertCSTSVariable)
            btcsts.grid();
            btcstsb = Button(lableframe1, text='BaseTOCSTS(B)', command=self.convertCSTSVariableBatch)
            btcstsb.grid();
            cstp = Button(lableframe1, text='CSTOPanel', command=self.cs2Panel)
            cstp.grid();
            ptcs = Button(lableframe1, text='PanelTOCS', command=self.panel2CS)
            ptcs.grid();
            lableframe2 = LabelFrame(self.d, text="Merge")
            lableframe2.pack()
            csdata = Button(lableframe2, text='CSData', command=self.readCSV_CS)
            csdata.grid();
            tsdata = Button(lableframe2, text='TSData', command=self.readCSV_TS)
            tsdata.grid();
            cstsdata = Button(lableframe2, text='CSTSData', command=self.readCSV_CSTS)
            cstsdata.grid();
            lableframe3 = LabelFrame(self.d, text="Join")
            lableframe3.pack()
            jcstsdata = Button(lableframe3, text='CSTSData', command=self.joinCSTS)
            jcstsdata.grid();
            jcsdata = Button(lableframe3, text='CSData', command=self.joinCS)
            jcsdata.grid();
            lableframe4 = LabelFrame(self.d, text="Finish")
            lableframe4.pack()
            finish= Button(lableframe4, text='Finish', command=self.quits)
            finish.grid()
            self.d.wait_window()
            ...
            self.openProject(newfile,0)
            if (self.tracestatus and trace):
                self.operatetrack.addOperate(self.istrackOperate,"Open Shapefile","createNewSTARSProject")
    ...
    def openProject(self,projectName=None,trace=1):
        ...
        if projectName:
            ...
            #    global project
                self.project = SProject(fin,self.master,self)
                self.project.directory = projectDir #取到project的存储路径，保存在project.directory
                ...
                self.enableMenus()
        if (self.tracestatus and trace):
            self.operatetrack.addOperate(self.istrackOperate,"Open Project","openProject")

    def saveProject(self,trace=1):
        self.project.writeProjectFile(self.project.name)
        if (self.tracestatus and trace):
            self.operatetrack.addOperate(self.istrackOperate,"Save Project","saveProject")

    def saveProjectAs(self,trace=1):
        fin = asksaveasfilename(filetypes=[("STARS Projects","*.prj")])
        fileName = fin 
        self.project.name = fileName
        self.saveProject(0)
        if (self.tracestatus and trace):
            self.operatetrack.addOperate(self.istrackOperate,"Save Project As","saveProjectAs")

    def projectSummary(self,trace=1):
        self.report(self.project.catalogue())
        if (self.tracestatus and trace):
            self.operatetrack.addOperate(self.istrackOperate,"Project Summary","projectSummary")
    ...
    def listVariablesPartial(self,trace=1):
        self.report(self.project.listVariablesPartial())
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate,"List Variable Names and Types","listVariablesPartial")
    ...
    def listVariable(self,trace=1):
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate,"List Variable","listVariable")

    def listVariablesNonInteractive(self,trace=1):
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate,"List Variables (Noninteractive)","listVariablesNonInteractive")
    ...
    def listIds(self,trace=1):
        self.report(self.project.listTimeIds())
        self.report(self.project.listRegionNames())
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate,"List Observation Ids","listIds")

    def listMatrices(self,trace=1):
        self.report("Matrices available: " + str(self.project.dataBase.matrices.keys()))
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate,"List Matrix Names","listMatrices")

    def listData(self):
        self.listVariables()
        self.listMatrices(0)

    def sLag(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        ...
        if d.status:
            ...
            self.projectSummary(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Spatial Lag", "sLag")

    def logTransform(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.projectSummary(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Log", "logTransform")

    def zTransform(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.projectSummary(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "z", "zTransform")

    def transform(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            if newName == "":
                ...
                self.projectSummary(0)
                if self.tracestatus and trace:
                    self.operatetrack.addOperate(self.istrackOperate, "Transform", "transform")

    def rawRate(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.projectSummary(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Raw Rate", "rawRate")

    def spatialRate(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.projectSummary(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Spatally Smoothed Rate", "spatialRate")

    def eBayesRate(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.projectSummary(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Empirical Bayes Rate", "eBayesRate")

    def cstsVariable(self,name,values):
        var = SVariable(values)

    def deleteVariable(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.projectSummary(0)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Delete", "deleteVariable")
            
    ######################################## 
    # matrix  callbacks
    ######################################## 
    
    def listMatrix(self,trace=1):
        ...
        if d.status:
            ...
            if wname == '':
                wname = "W_"+varName
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "List Matrix Values", "listMatrix")
    ...
    def distanceMatrix(self):
        ...
        if d.status: 
            ...
            self.listMatrices(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Distance based", "distanceMatrix")
    ...
    def regimeMatrix(self,trace=1):
        ...
        if d.status:
            ...
            self.listMatrices(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Regime based", "regimeMatrix")

    def gabriel(self,trace=1):
        if not hasattr(self.project,"distMatOn"):
            self.createDistanceMatrix()
        y = self.project.gabrielGraph()
        self.listMatrices(0)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Gabriel Proximity", "gabriel")
        
    def relativeProximity(self,trace=1):
        if not hasattr(self.project,"distMatOn"):
            self.createDistanceMatrix()
        y = self.project.relativeProximity()
        self.listMatrices(0)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Relative Proximity", "relativeProximity")

    def sphereOfInfluence(self,trace=1):
        if not hasattr(self.project,"distMatOn"):
            self.createDistanceMatrix()
        y = self.project.sphereOfInfluence()
        self.listMatrices(0)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Sphere of Influence Proximity", "sphereOfInfluence")

    def gis2Gal(self,trace=1):
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Gis Contiguity", "gis2Gal")

    def covarMatrix(self,trace=1):
        ...
        if d.status:
            ...
            self.listMatrices(0)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Covariance", "covarMatrix")
            
    def spatialMultiplier(self,trace=1):
        wNames = self.getMatrixNames()
            ...
            if sum([ x[1] for x in graph_options]):
                ...
                if graph_options[0][1]:
                    ...
                    self.drawMap(name,coords,multiplier[:,0],multiplier.name,timePeriod,
                                 poly2cs,cs2poly)
                if self.tracestatus and trace:
                    self.operatetrack.addOperate(self.istrackOperate, "Spatial Multiplier", "spatialMultiplier")

    def weightCharacteristics(self,trace=1):
        wNames = self.getMatrixNames()
        ...
        if d.status:
            ...
            h=SHistogram(timePeriodString,self.master, variable.name,
                         ...
                         bins=bins)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Characteristics", "weightCharacteristics")

    def deleteMatrix(self,trace=1):
        wNames = self.getMatrixNames()
        ...
        if d.status:
            ...
            self.projectSummary(0)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Delete", "deleteMatrix")
    ...
    def dscs(self,trace=1):
        ...
        if d.status:
            ...
            DV.MixedDataTable(top,values,name,
                              ...
                              fmt=fmt)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Cross section", "dscs")

    def dsts(self,trace=1):
        import sdialogue as sd
        ...
        if d.status:
            ...
            DV.MixedDataTable(top,values,name,
                              ...
                              fmt=fmt)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Time series", "dsts")
                

    def dspool(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CSTS_variable_names(exclude=['Time'])
        ...
        if d.status:
            ...
            if variable.varType == "CSTS":
                ...
                DV.MixedDataTable(top,values,name,
                                  ...
                                  fmt=fmt)
                if self.tracestatus and trace:
                    self.operatetrack.addOperate(self.istrackOperate, "Pooled", "dspool")

    def ddcs(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            DV.MixedDataTable(top,values,name,
                              ...
                              fmt=fmt)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Cross section", "ddcs")
            
    def ddts(self,trace=1):
        import sdialogue as sd
        varNames = self.get_TS_CSTS_variable_names(exclude=['Time'])
        ...
        if d.status:
            ...
            DV.MixedDataTable(top,values,name,
                              ...
                              fmt=fmt)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Time series", "ddts")
            
    def ddpool(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CSTS_variable_names(exclude=['Time'])
        ...
        if d.status:
            ...
            DV.MixedDataTable(top,values,name,
                              ...
                              fmt=fmt)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Pooled", "ddpool")
                

    ######################################## 
    # Esda callbacks
    ######################################## 
    def moran(self,trace=1):
        ...
        #计算问题中变量的莫兰指数
        entries = ['Variable'] #单行文本框，标题为Variable，获取左边添加的变量
        ...
        """RadioButtons表示类似checkbox的复选框，标题为Inference，有两种选项，分别是"Normality", 
        "Randomization".默认选中的是Normality"""

        values=[0,99,999,999,9999] 
        #Permutations所限制的取值范围。
        ...
        if d.status:
            ...
            if sum([ x[1] for x in graph_options]):
                ...
                if graph_options[0][1]: #graph_options是二维矩阵
                    coords = self.project.coords
                    poly2cs = self.project.poly2cs
                    cs2poly = self.project.cs2poly
                ...
                if graph_options[3][1]:
                    ...
                    b = SBoxPlot(varName,self.project,self.master,
                                  ...
                                  allX=variable)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Moran Global", "moran")
    def localMoran(self,trace=1):
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Moran Local", "localMoran")

    def geary(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Geary Global", "geary")


    def localGeary(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Geary Local", "localGeary")


    def globalG(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "G Global", "globalG")
            

    def localG(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "G Local", "localG")

    def traceTest(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        ...
        if d.status:
            ...
            if graph_options[3][1]:
                ...
                self.drawMap(varName,coords,variable[:,0],varName,timePeriod,
                             poly2cs,cs2poly)
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Trace Test", "traceTest")
            ...
            if sum([ x[1] for x in graph_options]):
                ...
                if graph_options[2][1]:              
                    ...
                    b = SBoxPlot(varName,self.project,self.master,
                                ...
                                allX=variable)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Classic Gini", "gini")
 
    def ginis(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
            ...
            if sum([ x[1] for x in graph_options]):
                ...
                if graph_options[2][1]:              
                    ...
                    b = SBoxPlot(varName,self.project,self.master,
                                  ...
                                  allX=variable)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Spatial Gini", "ginis")


    def theil(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
            ...
            if sum([ x[1] for x in graph_options]): 
                ...
                if graph_options[0][1] and variable.varType=="CSTS":
                    ...
                    ts = STimeSeries(varName, self.project, self.master, 
				                     ...
				                     yLabel="Global Theil")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Global Theil", "theil")
            
        
    def theild(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
            ...
            if sum([ x[1] for x in graph_options]):
                ...
                if graph_options[1][1]:              
                    ...
                    b = SBoxPlot(varName,self.project,self.master,
                                  ...
                                  allX=variable)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Theil Decomp", "theild")
    ...
    def tau(self,trace=1):
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Tau", "tau")


    def theta(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        regimeNames = self.get_CS_variable_names()
        ...
        if d.status:
            ...
            if graph_options[1][1]:
                ...
                self.drawMap("Theta Regime",coords,regime[:,0],regime.name,timePeriod,
                        ...
                        legendType="qualitative")
            if self.tracestatus and trace:
                self.operatetrack.addOperate(self.istrackOperate, "Theta", "theta")
            ...
            if sum([ x[1] for x in graph_options ]):
                ...
                if graph_options[1][1]: 
                    ...
                    self.drawMap("Convergence Classification (End Points)",coords,variableE,
                                ...
                                legendType="diverging")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Classic", "markov")


    def smarkov(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CSTS_variable_names(exclude=['Time'])
            ...
            if sum([ x[1] for x in graph_options ]):
                ...
                if graph_options[1][1]: 
                    ...
                    self.drawMap("Convergence Classification (End Points)",coords,variableE,
                                ...
                                legendType="diverging")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Spatial", "smarkov")

            
   
    def localTrans(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CSTS_variable_names(exclude=['Time'])
            ...
            if sum([ x[1] for x in graph_options ]):
                ...
                if graph_options[1][1]: 
                    ...
                    self.drawMap("Convergence Classification (End Points)",coords,variableE,
                                ...
                                legendType="diverging")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Local Moran", "localTrans")



    def kmeans(self,trace=1):
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            self.drawMap("K-Means",coords,variable[:,0],varName,timePeriod,
                           ...
                           legendType="qualitative")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Kmeans", "kmeans")
    ...
    def quantileMap(self,trace=1):
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Quantile", "quantileMap")
                
    def stdmap(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Std. Deviation", "stdmap")
    
    def divergingMap(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                self.drawMap(timePeriodString,coords,y,varName,t,
                     ...
                     legendType="diverging")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Diverging", "divergingMap")

    def userDefinedC(self,trace=1):
        import sdialogue as sd
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                self.drawMap(timePeriodString,coords,y,varName,t,
                     ...
                     legendType='sequential')
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Sequential User Defined", "userDefinedC")
        

    def regimeMap(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Regime", "regimeMap")

    def userDefinedD(self,trace=1):
        import sdialogue as sd
        varNames = tuple(self.get_CS_CSTS_variable_names(exclude=['Time']))
        timePeriods = self.project.timeString
            ...
            for t in timePeriods:
                ...
                self.drawMap(timePeriodString,coords,y,varName,t,
                     poly2cs,cs2poly,classification="userDefined",bins=bins,
                             legendType="qualitative")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Qualitative User Defined", "userDefinedD")

    def boxMap(self,trace=1):

        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        d = sd.SDialogue('Box Map Dialogue') #添加了对话框的标题名为Box Map Dialogue
        txt="The variable to be mapped."
        entries = ['Variable'] #Entry,单行文本框，用于收集键盘输入。此处设置标题为'Variable'
        sd.MultiEntry(d,varNames, entries, title='Variable to map',
                      helpText=txt) 
        #MultiEntry表示的是Box Map Dialog对话框中的Variable to Map部分的设计与实现
        txt="Select one or more time periods for map(s)."
        sd.DualListBoxes(d,timePeriods,title='Time Periods', helpText=txt) 
        #DualListBoxes表示的是Box Map Dialog对话框中的Time Periods部分的设计与实现
        d.draw()
        if d.status:
            varName = d.results[0]['Variable']
            variable = self.project.dataBase.getVariable(varName)
            ...
            for t in timePeriods:
                ...
                self.drawMap(timePeriodString,coords,y,varName,t,
                             ...
                             legendType="sequential")
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Box Map", "boxMap")

    def rescaleMapCoords(self):
        self.project.reScaleCoords()

    def scatter(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                s.updateTitle(name)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Scatter Plot", "scatter")

    def cscatter(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            CScatterPlot("csp",self.master,x,y,z,xLabel=varNameX,yLabel=varNameY,
                zLabel = varNameZ)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Conditional Scatter Plot", "cscatter")

    def histogramSturges(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                h = SHistogram(timePeriodString,self.master,varName,
                        ...
                        tsIds = t)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Sturges Rule", "histogramSturges")

    def histogramEqualWidth(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        d = sd.SDialogue('Histogram Dialogue')
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                h = SHistogram(timePeriodString,self.master,varName,
                        ...
                        nBins = nBins)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Equal Width", "histogramEqualWidth")

    def histogramUserDefined(self,trace=1):
        import sdialogue as sd
        varNames = self.get_CS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        d = sd.SDialogue('Histogram Dialogue')
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                h = SHistogram(timePeriodString,self.master,varName,
                        ...
                        bins=bins)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "User Defined", "histogramUserDefined")
                
    def timeSeriesCs(self,trace=1):
        import sdialogue as sd
        varNames = tuple(self.get_CS_CSTS_variable_names(exclude=['Time']))
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            for csid in csids:
                ...
                ts = STimeSeries(varName,
                            ...
                            yLabel=yLabel)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Regional Time Series", "timeSeriesCs")

    def timeSeries(self,trace=1):
        import sdialogue as sd
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Time Series", "timeSeries")

    def timePath(self,trace=1):
        import sdialogue as sd
        varNames = self.get_TS_CSTS_variable_names(exclude=['Time'])
        timePeriods = self.project.timeString
        regions = self.project.regionNames
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Time Path", "timePath")

    def boxPlot(self,trace=1):
        import sdialogue as sd
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                h = SBoxPlot(varName,self.project,self.master,
                        ...
                        fence = fence)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Box Plot", "boxPlot")

    def density(self,trace=1):
        import sdialogue as sd
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Density", "density")

    def cdf(self,trace=1):
        import sdialogue as sd
        varNames = tuple(self.get_CS_CSTS_variable_names(exclude=['Time']))
        timePeriods = self.project.timeString
        ...
        if d.status:
            ...
            for t in timePeriods:
                ...
                s = SCDF("CDF",self.master,
                    ...
                    title=title,xLabel=varName,xmin=xmin,xmax=xmax)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "CDF", "cdf")
                
    def pcp(self,trace=1):
        import sdialogue as sd
        varNames = tuple(self.get_CS_CSTS_variable_names(exclude=['Time']))
        timePeriods = self.project.timeString
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Parallel Coordinate Plot", "pcp")

    def spaceTimeButtonMatrix(self,trace=1):
        import sdialogue as sd
        varNames = tuple(self.get_CS_CSTS_variable_names(exclude=['Time']))
        timePeriods = self.project.timeString
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Space Time Button Matrix", "spaceTimeButtonMatrix")
...
#################### End of Visualization Menu Callbacks ###################
    ...
    def iconify(self,trace=1):
        for key in viewObserver.items.keys():
            view = viewObserver.items[key]
            view.iconify()
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Iconify All", "iconify")

    def deiconify(self,trace=1):
        for key in viewObserver.items.keys():
            view = viewObserver.items[key]
            view.deiconify()
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "DeIconify All", "deiconify")


    def raiseAll(self,trace=1):
        self.iconify()
        self.deiconify(0)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Raise All", "raiseAll")
    def arrangeAll(self):
        self.report("arrangeAll called")

    def closeWindows(self,trace=1):
        for key in viewObserver.items.keys():
            view = viewObserver.items[key]
            view.Quit()
        self.project.closeController()
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Close All", "closeWindows")
    def listViewsE(self,event):
        self.listViews(0)
    def listViews(self,trace=1):
        lnames = {}
        for key in viewObserver.items.keys():
            view = viewObserver.items[key]
        ...
        for item in keys:
            s.append(item)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "List Views", "listViews")
    ...
    def createViewController(self,trace=1):
        try:
        ...
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "View Controller", "createViewController")

    ######################################## 
    # help callbacks
    ######################################## 
    def userManual(self,trace=1):
        ...
        showerror("Info",txt)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "User Manual", "userManual")
    ...
    def about(self,trace=1):
        ss=sd.About(self.master)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "About", "about")

    def gcredits(self,trace=1):
        ss=sd.Credits(self.master)
        if self.tracestatus and trace:
            self.operatetrack.addOperate(self.istrackOperate, "Credits  ", "gcredits")
    def tutorial(self):
    ...
    def startTrack(self):
        self.operatetrack = OperateTrack.OperateTrack()
        self.istrackOperate = 1
        self.report("Start Operate Track!")
        self.tracestatus=1

    def finishTrack(self):
        if self.tracestatus:
            self.operatetrack.saveOperate()
            self.istrackOperate = 0
            self.report("Finish Operate Track!")
            self.tracestatus=0
