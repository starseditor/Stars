...
class SDialogue:
    ...
    def draw(self):
        ...
        #Frame指包含其他控件的纯容器，该容器包含了下面的ok与cancel两个按钮
        ok = Tk.Button(buttonFrame, command=self.onOk, text="OK") #可视化界面里面的OK按钮
        cancel = Tk.Button(buttonFrame, command=self.onCancel, 
                     text="Cancel") #可视化界面里的cancel按钮
...
class DualListBoxes(ChildWidget):
    ...
    #左右列表框滚动条对。 左用于选择发送到右侧lb-sb对的项目。
    def __init__(self,parent,optionList,title=None,helpText=None,
            conditional=None):
        self.parent = parent
        self.top = self.parent.top #继承父类距离顶部的位置
        self.optionList = optionList #选项列表
        self.title=title
        self.helpText = helpText
    ...
    def draw(self):
        ...
        self.leftLb = Tk.Listbox(lFr,height=6,width=16,background="yellow")
